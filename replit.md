...
- June 25, 2025: Implemented context-aware TimelineComposer interface - added defaultMode prop to display task creation interface (green CheckSquare icon, "What needs to be done?" placeholder) when on Tasks tab, while maintaining comment interface for other tabs, ensuring users immediately see appropriate creation mode based on selected tab context.
