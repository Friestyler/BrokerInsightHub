...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Additional dynamic routes for detailed endpoints
  app.get('/api/:envId/partners/:partnerId/customers', async (req, res) => {
    try {
      ...
      console.error(`Error fetching partner customers for ${req.params.envId}:`, error);
      res.json([]);
    }
  });

  app.get('/api/:envId/partners/:partnerId/opportunities', async (req, res) => {
    try {
      const { envId, partnerId } = req.params;
      ...
      const result = await envPool.query(`
        SELECT o.*
        FROM ${envId}.opportunities o
        INNER JOIN ${envId}.partner_opportunities po ON o.id = po.opportunity_id
      ...
      console.error(`Error fetching partner opportunities for ${req.params.envId}:`, error);
      res.json([]);
    }
  });
  ...
  app.get('/api/:envId/saved-lists', async (req, res) => {
    try {
      ...
      const envPool = getEnvironmentPool(envId);
      const result = await envPool.query(`
        SELECT * FROM ${envId}.saved_lists ORDER BY id
      `);
      
      res.json(result.rows);
    } catch (error) {
      console.error(`Error fetching saved lists for ${req.params.envId}:`, error);
      res.json([]);
    }
  });

  app.get('/api/:envId/saved-views', async (req, res) => {
    try {
      const { envId } = req.params;
      ...
      const envPool = getEnvironmentPool(envId);
      const result = await envPool.query(`
        SELECT * FROM ${envId}.saved_views ORDER BY id
      `);
      
      res.json(result.rows);
    } catch (error) {
      console.error(`Error fetching saved views for ${req.params.envId}:`, error);
      res.json([]);
    }
  });

  app.get('/api/:envId/okr-metrics', async (req, res) => {
    try {
      const { envId } = req.params;
      ...
      const envPool = getEnvironmentPool(envId);
      const result = await envPool.query(`
        SELECT * FROM ${envId}.okr_metrics ORDER BY id
      `);
      
      res.json(result.rows);
    } catch (error) {
      console.error(`Error fetching OKR metrics for ${req.params.envId}:`, error);
      res.json([]);
    }
  });

  app.get('/api/:envId/okr-tags', async (req, res) => {
    try {
      const { envId } = req.params;
      ...
      const envPool = getEnvironmentPool(envId);
      const result = await envPool.query(`
        SELECT * FROM ${envId}.okr_tags ORDER BY id
      `);
      
      res.json(result.rows);
    } catch (error) {
      console.error(`Error fetching OKR tags for ${req.params.envId}:`, error);
      res.json([]);
    }
  });

  app.get('/api/:envId/template-assignments/partner/:partnerId?', async (req, res) => {
    try {
      const { envId, partnerId } = req.params;
      ...
      const envPool = getEnvironmentPool(envId);
      let query = `SELECT * FROM ${envId}.template_assignments`;
      let params: any[] = [];
      
      if (partnerId) {
        query += ` WHERE entity_type = 'partner' AND entity_id = $1`;
        params = [partnerId];
      }
      ...
      console.error(`Error fetching template assignments for ${req.params.envId}:`, error);
      res.json([]);
    }
  });
