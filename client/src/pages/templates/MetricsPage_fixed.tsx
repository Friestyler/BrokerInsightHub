...
// Component for creating a new metric form (same as before)
...
// Component for creating a new group form (same as before)
...
export default function MetricsPage() {
  ...
  // Get a metric's parent if it exists
  const getParentMetric = (metric: any) => {
    if (!metric.parent) return null;
    return metrics.find(m => m.id === metric.parent);
  };
  
  // Get hierarchical tree structure for display
  const getHierarchicalMetrics = () => {
    // First get all top level objectives
    const objectives = filteredMetrics.filter(m => m.hierarchy === "objective");
    
    // For each objective, get its activities
    const result = objectives.map(objective => {
      const activities = filteredMetrics.filter(
        m => m.parent === objective.id && m.hierarchy === "activity"
      );
      
      // For each activity, get its subactivities
      const activitiesWithChildren = activities.map(activity => {
        const subactivities = filteredMetrics.filter(
          m => m.parent === activity.id && m.hierarchy === "subactivity"
        );
                ...
                <TableBody>
                  {getHierarchicalMetrics().map(objective => (
                    <React.Fragment key={`objective-${objective.id}`}>
                            ...
                            {objective.activities?.length > 0 && (
                              ...
                              >
                                <svg 
                                  ...
                                  className={`transition-transform ${expandedItems.includes(objective.id) ? 'rotate-90' : ''}`}
                                >
                      ...
                      {/* Activity rows */}
                      {expandedItems.includes(objective.id) && objective.activities.map((activity: any) => (
                        <React.Fragment key={`activity-${activity.id}`}>
                                ...
                                {activity.subactivities?.length > 0 && (
                                  ...
                                  >
                                    <svg 
                                      ...
                                      strokeLinecap="round" 
                                      strokeLinejoin="round"
                                      className={`transition-transform ${expandedItems.includes(activity.id) ? 'rotate-90' : ''}`}
                                    >
                                      <polyline points="9 18 15 12 9 6"></polyline>
                                    </svg>
                                  </Button>
                          ...
                          {/* Subactivity rows */}
                          {expandedItems.includes(activity.id) && activity.subactivities.map((subactivity: any) => (
                            <TableRow 
