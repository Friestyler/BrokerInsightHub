  ...
  <>
    <div className="mb-10 mx-auto max-w-6xl">
      ...
      <div className="mt-8 mx-auto max-w-6xl">
        <h2 className="text-xl font-semibold text-gray-900 mb-6">Platform Activity Overview</h2>
        
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {/* Comments Card */}
          <Card className="border border-[#E6E7F1] hover:border-[#5567E5] transition-colors">
            <CardHeader className="pb-3">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 bg-[#E6E7F1] rounded-lg flex items-center justify-center">
                    <MessageSquare className="h-4 w-4 text-[#5567E5]" />
                  </div>
                  <CardTitle className="text-lg text-gray-900">Comments</CardTitle>
                </div>
                <Dialog>
                  <DialogTrigger asChild>
                    <Button variant="ghost" size="sm" className="h-8 w-8 p-0 hover:bg-[#F5F6FE]">
                      <Expand className="h-4 w-4 text-[#5567E5]" />
                    </Button>
                  </DialogTrigger>
                  <DialogContent className="max-w-4xl max-h-[80vh]">
                    <DialogHeader>
                      <DialogTitle className="flex items-center gap-2">
                        <MessageSquare className="h-5 w-5 text-[#5567E5]" />
                        All Platform Comments
                      </DialogTitle>
                    </DialogHeader>
                    <ScrollArea className="h-[60vh] pr-4">
                      <div className="space-y-4">
                        {unifiedActivities?.filter((activity: any) => activity.activity_type === 'comment').map((comment: any) => (
                          <div key={comment.id} className="bg-white border border-gray-200 rounded-lg p-4">
                            <div className="flex items-start gap-3">
                              <div className="w-8 h-8 bg-[#5567E5] rounded-full flex items-center justify-center text-white text-sm font-medium">
                                {users?.find((u: any) => u.id === comment.user_id)?.initials || 'U'}
                              </div>
                              <div className="flex-1">
                                <div className="flex items-center gap-2 mb-1">
                                  <span className="font-medium text-gray-900">
                                    {users?.find((u: any) => u.id === comment.user_id)?.name || 'Unknown User'}
                                  </span>
                                ...
                                <div className="mt-2 text-xs text-gray-500">
                                  {comment.entity_type}: {comment.entity_name || `ID ${comment.entity_id}`}
                                </div>
                              </div>
                            </div>
                          </div>
            ...
            <CardContent>
              <div className="space-y-3">
                <div className="text-2xl font-bold text-[#5567E5]">
                  {unifiedActivities?.filter((activity: any) => activity.activity_type === 'comment').length || 0}
                </div>
                <div className="space-y-2">
                  {unifiedActivities?.filter((activity: any) => activity.activity_type === 'comment')
                    .slice(0, 2).map((comment: any) => (
                    <div key={comment.id} className="text-sm">
                      <div className="flex items-center gap-2 mb-1">
                        <div className="w-4 h-4 bg-[#5567E5] rounded-full flex items-center justify-center text-white text-xs">
                          {users?.find((u: any) => u.id === comment.user_id)?.initials || 'U'}
                        </div>
                      ...
                      <p className="text-gray-600 text-xs truncate pl-6">{comment.content}</p>
                    </div>
                  )) || (
          ...
          <Card className="border border-[#E6E7F1] hover:border-[#5567E5] transition-colors">
            <CardHeader className="pb-3">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 bg-[#E6E7F1] rounded-lg flex items-center justify-center">
                    <CheckSquare className="h-4 w-4 text-[#5567E5]" />
                  </div>
                  <CardTitle className="text-lg text-gray-900">Tasks</CardTitle>
                </div>
                <Dialog>
                  <DialogTrigger asChild>
                    <Button variant="ghost" size="sm" className="h-8 w-8 p-0 hover:bg-[#F5F6FE]">
                  ...
                  <DialogContent className="max-w-4xl max-h-[80vh]">
                    <DialogHeader>
                      <DialogTitle className="flex items-center gap-2">
                        <CheckSquare className="h-5 w-5 text-[#5567E5]" />
                        All Platform Tasks
                      </DialogTitle>
                    </DialogHeader>
                    <ScrollArea className="h-[60vh] pr-4">
                      <div className="space-y-4">
                        {unifiedActivities?.filter((activity: any) => activity.activity_type === 'task').map((task: any) => (
                          <div key={task.id} className="bg-white border border-gray-200 rounded-lg p-4">
                            <div className="flex items-start gap-3">
                              <div className="w-8 h-8 bg-[#5567E5] rounded-full flex items-center justify-center text-white text-sm font-medium">
                                {users?.find((u: any) => u.id === (task.assigned_to || task.user_id))?.initials || 'U'}
                              </div>
                              <div className="flex-1">
                                ...
                                <div className="flex items-center gap-4 text-xs text-gray-500">
                                  <span>Assigned to: {users?.find((u: any) => u.id === task.assigned_to)?.name || 'Unassigned'}</span>
                                  <span>{new Date(task.created_at).toLocaleDateString()}</span>
                                  <span>{task.entity_type}: {task.entity_name || `ID ${task.entity_id}`}</span>
                                </div>
                              </div>
                            </div>
                          </div>
            ...
            <CardContent>
              <div className="space-y-3">
                <div className="text-2xl font-bold text-[#5567E5]">
                  {unifiedActivities?.filter((activity: any) => activity.activity_type === 'task').length || 0}
                </div>
                <div className="space-y-2">
                  {unifiedActivities?.filter((activity: any) => activity.activity_type === 'task')
                    .slice(0, 2).map((task: any) => (
                    <div key={task.id} className="text-sm">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-gray-600 text-xs">
                          {users?.find((u: any) => u.id === task.assigned_to)?.name || 'Unassigned'}
                        </span>
                      </div>
                    </div>
                  )) || (
          ...
          <Card className="border border-[#E6E7F1] hover:border-[#5567E5] transition-colors">
            <CardHeader className="pb-3">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 bg-[#E6E7F1] rounded-lg flex items-center justify-center">
                    <FileText className="h-4 w-4 text-[#5567E5]" />
                  </div>
                ...
                <Dialog>
                  ...
                  <DialogContent className="max-w-4xl max-h-[80vh]">
                    ...
                    <ScrollArea className="h-[60vh] pr-4">
                      <div className="space-y-4">
                        {unifiedActivities?.filter((activity: any) => activity.activity_type === 'attachment').map((doc: any) => (
                          <div key={doc.id} className="bg-white border border-gray-200 rounded-lg p-4">
                            <div className="flex items-start gap-3">
                              <div className="w-8 h-8 bg-[#5567E5] rounded-lg flex items-center justify-center">
                                <FileText className="h-4 w-4 text-white" />
                              </div>
                              <div className="flex-1">
                                <div className="flex items-center justify-between mb-2">
                                  <h3 className="font-medium text-gray-900">{doc.title || doc.content}</h3>
                                  <span className="text-sm text-gray-500">
                                ...
                                <div className="flex items-center gap-4 text-xs text-gray-500">
                                  <span>Added by: {users?.find((u: any) => u.id === doc.user_id)?.name || 'Unknown User'}</span>
                                  <span>{doc.entity_type}: {doc.entity_name || `ID ${doc.entity_id}`}</span>
                                </div>
                              </div>
                            </div>
                          </div>
            ...
            <CardContent>
              <div className="space-y-3">
                <div className="text-2xl font-bold text-[#5567E5]">
                  {unifiedActivities?.filter((activity: any) => activity.activity_type === 'attachment').length || 0}
                </div>
                <div className="space-y-2">
                  {unifiedActivities?.filter((activity: any) => activity.activity_type === 'attachment')
                    .slice(0, 2).map((doc: any) => (
                    <div key={doc.id} className="text-sm">
                      <div className="flex items-center gap-2 mb-1">
                        <FileText className="h-3 w-3 text-[#5567E5]" />
                        <span className="font-medium text-gray-700 truncate">{doc.title || doc.content}</span>
                      </div>
                      <div className="flex items-center gap-2">
